<template>
  <section>
    <button v-show="tableIsVisible" v-on:click="refreshList">Refresh List</button>
    <table v-show="tableIsVisible" v-if="users.length > 0" class="users-tabel">
      ...
      <tbody>
        ...
        >
          <td class="users-tabel__data users-tabel__data--number">{{ user.id }}</td>
          ...
          <td v-if="user.address" class="users-tabel__data users-tabel__data--address">
            <ul>
    ...
    <section v-if="users.length > 0">
      <span>Total Users Number:</span> <span class="badge success">{{ totalUsersNumber }}</span>
    </section>

    <button
      v-if="users.length > 0"
      v-on:click="toggleTableState()"
      v-tooltip.top-center="tooltipMessage"
    >
      <span v-if="tableIsVisible">Hide tabel</span> <span v-else>Show tabel</span>
    </button>
